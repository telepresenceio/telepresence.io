...
export async function getDemoClusterMetadata(dispatch, claim, origin) {
  try {
    const apiUrl = getBaseUrl();
    const originParam = origin ? '&origin=' + encodeURIComponent(origin) : '';

    const response = await retry(() =>
      fetch(`${apiUrl}/democlusters/${TELEPRESENCE_DEMO_CLUSTER_ID}?autoClaim=${claim}${originParam}`, requestOptions));
