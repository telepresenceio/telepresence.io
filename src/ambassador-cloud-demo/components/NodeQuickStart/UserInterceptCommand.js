...
const UserInterceptCommand = () => {
  const SERVICES_REFRESH_INTERVAL = 5_000;
  const dispatch = useAppDispatch();
  const { userInfo, userServices } = useAppState();

  const userIntercept = userServices?.find((service) => service.intercepts && service.intercepts[service.intercepts.length-1]?.name === telepresenceIntercept);
  ...
  const interceptCommand = `$ telepresence intercept ${telepresenceIntercept} --port  8080`;

  const intercepted = interceptId && previewURL ? `\tUsing deployment ${telepresenceIntercept}\n` +
    "\tintercepted\n" +
    `\t    Intercept name         : ${telepresenceIntercept}\n` +
    "\t    State                  : ACTIVE\n" +
